...
CREATE TABLE IF NOT EXISTS TBL_USERS (
  id___  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  email  VARCHAR(255) NOT NULL UNIQUE,
  role_  VARCHAR(20)  NOT NULL DEFAULT 'app_user'
                      CHECK (role_ IN ('app_user', 'app_admin')),
  ctime  TIMESTAMPTZ  NOT NULL DEFAULT NOW()   -- created_at (5文字制約外: 監査用共通)
);

COMMENT ON TABLE  TBL_USERS       IS 'ユーザマスタ';
COMMENT ON COLUMN TBL_USERS.id___ IS 'ユーザUUID (PK)';
COMMENT ON COLUMN TBL_USERS.email IS 'メールアドレス';
COMMENT ON COLUMN TBL_USERS.role_ IS 'アプリケーションロール';
...
CREATE TABLE IF NOT EXISTS TBL_CATEG (
  id___  UUID         PRIMARY KEY DEFAULT gen_random_uuid(),
  ownid  UUID         NOT NULL REFERENCES TBL_USERS(id___) ON DELETE CASCADE,
...
COMMENT ON COLUMN TBL_CATEG.id___ IS '科目UUID (PK)';
...
CREATE TABLE IF NOT EXISTS TBL_TRANS (
  id___  UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
  ownid  UUID          NOT NULL REFERENCES TBL_USERS(id___) ON DELETE CASCADE,
  catid  UUID          NOT NULL REFERENCES TBL_CATEG(id___) ON DELETE RESTRICT,
...
COMMENT ON COLUMN TBL_TRANS.id___ IS '明細UUID (PK)';
...
CREATE TABLE IF NOT EXISTS TBL_AUDIT (
  id___  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  ownid  UUID        REFERENCES TBL_USERS(id___) ON DELETE SET NULL,
  actio  VARCHAR(200) NOT NULL,           -- 例: "CREATE_TRANS:uuid" "DELETE_CATEG:uuid"
  ctime  TIMESTAMPTZ  NOT NULL DEFAULT NOW()
);
...
COMMENT ON COLUMN TBL_AUDIT.id___ IS '監査ログUUID (PK)';
